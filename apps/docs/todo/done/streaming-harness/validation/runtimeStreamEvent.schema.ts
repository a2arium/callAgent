...
export const STREAM_EVENT_VERSION = '2026-05-02' as const;
...
export const RuntimeStreamEnvelopeBaseSchema = z.object({
  version: z.literal(STREAM_EVENT_VERSION),
  id: z.string().min(1),
...
const FilePartSchema = ImagePartSchema.extend({
  type: z.literal('file'),
}).strict();

const AudioPartSchema = ImagePartSchema.extend({
  type: z.literal('audio'),
}).strict();

const VideoPartSchema = ImagePartSchema.extend({
...
export type TaskStatusEvent = Extract<RuntimeStreamEvent, { type: 'task.status' }>;

export function isTerminalTaskStatus(event: RuntimeStreamEvent): event is TaskStatusEvent {
  return (
