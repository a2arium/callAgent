import type { BridgeTaskInput, ChatRoute, Invoker, ResultPayload } from '../types.js';
...
export class JsonRpcInvoker implements Invoker {
    ...
    async start(params: { id: string; input: BridgeTaskInput; agentId: string; tenantId?: string; route: ChatRoute }): Promise<ResultPayload> {
    ...
    async resume(params: { id: string; token: string; input: BridgeTaskInput; tenantId?: string; route: ChatRoute }): Promise<ResultPayload> {
