...
export async function createWorkspace(name: string, options: GenerateOptions): Promise<{ output: string; sources: string[] }> {
    ...
    await write(output, 'package.json', JSON.stringify({ name, private: true, scripts: { start: 'callagent start', dev: 'callagent start', validate: 'callagent workspace validate', agents: 'callagent agents list', 'db:setup': 'callagent db setup', 'db:migrate': 'callagent db migrate', 'infra:up': 'callagent infra up', 'infra:down': 'callagent infra down', 'infra:restart': 'callagent infra restart', 'maintenance:status': 'callagent maintenance status', 'maintenance:run': 'callagent maintenance run' }, devDependencies: { '@a2arium/callagent-cli': packageRanges.cli }, dependencies: { '@a2arium/callagent-runtime': packageRanges.runtime } }, null, 2) + '\n');
    const maintenanceInstallationId = randomUUID();
    const runtimeInstallationId = randomUUID();
    await write(output, '.env.example', `# Shared runtime infrastructure\nMEMORY_DATABASE_URL=postgres://callagent:callagent@localhost:5432/callagent\nNATS_URL=nats://localhost:4222\n\n# Hatchet (create an API token at http://127.0.0.1:8080 for the local POC)\nCALLAGENT_OUTBOX_DISPATCHER=hatchet\nCALLAGENT_RUNTIME_INSTALLATION_ID=${runtimeInstallationId}\nCALLAGENT_HATCHET_WORKER_READINESS=required\nHATCHET_CLIENT_TOKEN=replace-with-hatchet-api-token\nHATCHET_CLIENT_HOST_PORT=localhost:7077\nHATCHET_CLIENT_TLS_STRATEGY=none\n# Change this when another local project already uses 8080.\nHATCHET_DASHBOARD_PORT=8080\nHATCHET_DASHBOARD_URL=http://127.0.0.1:8080\nHATCHET_DASHBOARD_TENANT_ID=707d0855-80ab-4e1f-a156-f1c4546cbf52\n\n# Observer authentication\nCALLAGENT_PUBLIC_URL=http://127.0.0.1:8790\nBETTER_AUTH_SECRET=replace-with-at-least-32-random-characters\nCALLAGENT_OPERATOR_ENABLED=true\nCALLAGENT_OPERATOR_BOOTSTRAP_EMAIL=admin@callagent.local\nCALLAGENT_OPERATOR_BOOTSTRAP_TENANT_ID=default\n# Optional: set a known first-login password; otherwise one is printed once at startup.\n# CALLAGENT_OPERATOR_BOOTSTRAP_PASSWORD=\n`);
    await fs.appendFile(path.join(output, '.env.example'), `\n# Workspace-owned automatic maintenance; use one owner for a shared tenant/database policy.\nCALLAGENT_MAINTENANCE_INSTALLATION_ID=${maintenanceInstallationId}\nCALLAGENT_MAINTENANCE_OWNER=true\nCALLAGENT_MAINTENANCE_TENANT_ID=default\nCALLAGENT_MAINTENANCE_EXPIRY_CRON=17 * * * *\nCALLAGENT_MAINTENANCE_RETENTION_CRON=23 3 * * *\n# Debug retention is report-only until explicitly enabled.\nCALLAGENT_RETENTION_APPLY=false\n`);
    await fs.appendFile(path.join(output, '.env.example'), `\n# Durable, fenced live-progress projection for Operator.\nCALLAGENT_RUN_PROGRESS=enabled\n`);
    await fs.appendFile(path.join(output, '.env.example'), `\n# Abort and drain active worker callbacks before process replacement.\nCALLAGENT_WORKER_SHUTDOWN_GRACE_MS=30000\n`);
    await write(output, '.gitignore', '.env\n.callagent/runtime/\n');
    await write(output, 'README.md', workspaceReadme(name));
    await fs.appendFile(path.join(output, 'README.md'), `\n## Maintenance\n\nThis workspace owns automatic maintenance when \`CALLAGENT_MAINTENANCE_OWNER=true\`. The owner reconciles hourly expiry cleanup and daily retention review in Hatchet.\n\n\`\`\`bash\nnpm run maintenance:status # read-only candidate report\nnpm run maintenance:run    # immediate cleanup; retention follows apply policy\n\`\`\`\n\nKeep one owner for every shared Hatchet tenant and database policy; set \`CALLAGENT_MAINTENANCE_OWNER=false\` in other workspaces. Expired cache entries, including current 30-day artifact entries, are deleted automatically. Debug retention remains report-only unless \`CALLAGENT_RETENTION_APPLY=true\`.\n`);
    await fs.appendFile(path.join(output, 'README.md'), `\n## Live progress\n\nLong-running agents may publish bounded Operator progress with \`ctx.progress.report(...)\` after committing a domain checkpoint. It is enabled by \`CALLAGENT_RUN_PROGRESS=enabled\` and appears independently of the run graph.\n`);
    await fs.appendFile(path.join(output, 'README.md'), `\n## Worker recovery\n\nIf Hatchet no longer reports this exact worker as active, CallAgent stops dispatch reconciliation, aborts active agent segments, and drains them for \`CALLAGENT_WORKER_SHUTDOWN_GRACE_MS\` (30 seconds by default). The worker then exits non-zero so Docker, systemd, or Kubernetes can replace the complete process. Deploy matching CallAgent runtime packages together; do not run mixed worker protocol versions. Put external mutations behind registered effects and stable idempotency keys.\n`);
