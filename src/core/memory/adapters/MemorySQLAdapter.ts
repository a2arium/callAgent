...
export class MemorySQLAdapter implements IMemory {
    ...
    async delete(key: string): Promise<void> {
        try {
            ...
            // Check for PrismaClientKnownRequestError with code P2025 (record not found)
     ...
     */
    async query<T>(opts: MemoryQueryOptions): Promise<Array<MemoryQueryResult<T>>> {
        if (opts.similarityVector) {
            throw new MemoryError('Vector search is not supported by this adapter version.',
                { code: 'NOT_IMPLEMENTED' });
        }

        const findOptions: any = {}; // Will be properly typed when using the generated Prisma types
        ...
        if (opts.filters && opts.filters.length > 0) {
            try {
                ...
                const filterConditions = opts.filters.map(filter =>
                    this.buildJsonFilterCondition(filter)
                );

                // Add filter conditions with AND logic
                whereConditions.AND = whereConditions.AND || [];
                whereConditions.AND.push(...filterConditions);
            } catch (error: any) {
