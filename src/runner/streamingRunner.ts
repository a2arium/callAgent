    ...
    try {
        ...
        if (isDevMode) {
            ...
            if (!agentModulePath.includes('dist')) {
                // Send warning to stderr if format is json or sse to avoid breaking pipes
                if (options.outputType === 'json' || options.outputType === 'sse') {
                    console.error(`[StreamingRunner] Warning: In production mode, agent paths should typically be from the 'dist' directory.`, {
